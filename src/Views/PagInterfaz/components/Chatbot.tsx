...
export default function Chatbot() {
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      // Datos a agregar
      const nuevoPrompt = {
        prompt: `Eres un organizador de eventos y quiero que respondas al siguiente prompt de un usario: ${input.trim()}`,
        creadoEn: new Date()
      };
      // Agregar documento
    ...
    <div className="fixed bottom-4 right-4 z-50">
      {isOpen ? (
        <div className="bg-white rounded-lg shadow-xl w-80 h-96 flex flex-col">
          <div className="bg-gradient-to-r from-[#FDD46B] to-[#FE9748] text-black p-4 rounded-t-lg flex justify-between items-center">
          ...
          <div className="flex-1 p-4 overflow-y-auto">
            ...
            <div className="chat-container h-96 p-4">
              {messages.map((msg, index) => (
                ...
                >
                  <div className={`inline-flex text-gray-800 p-3 rounded-lg mb-2 message ${msg.sender === 'bot' ? 'bg-[#FE8C40]' : 'bg-gray-100'}`}>{msg.text}</div>
                </div>
              ))}
          ...
          <div className="p-4 border-t">
            <div className="flex items-center space-x-2">
              <input
                ...
                className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#FE9748]"
                value={input}
              ...
              <button
                onClick={handleSend}
                className="bg-[#FE9748] text-white p-3 rounded-full shadow-lg hover:bg-[#FE8C40] transition"
              >Enviar</button>
            </div>
          </div>
        </div>
